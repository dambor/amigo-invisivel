...
export const ResultList: React.FC<ResultListProps> = ({ group, template, onReset }) => {
  ...
  // Atualizado: Link genérico do grupo, sem UID, para cair na tela de Login/Feed
  const generateGroupLink = () => {
    const params = new URLSearchParams();
    params.set('gid', group.id);
    // Não passamos UID nem action específica, o App.tsx vai detectar e mandar para o Login
    return `${generateBaseUrl()}?${params.toString()}`;
  };
  ...
  const getWhatsappLink = (p: Participant) => {
    ...
    const groupLink = generateGroupLink(); // Link único para o grupo todo
