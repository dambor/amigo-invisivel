...
import { Send, Share2, EyeOff, CheckCircle, ArrowRight, List, PlayCircle } from 'lucide-react';
...
export const ResultList: React.FC<ResultListProps> = ({ participants, template, onReset }) => {
  ...
  const getWhatsappLink = (p: Participant) => {
    ...
    const message = template
      ...
      .replace(/{{AMIGO}}/g, `\n*Toque aqui para ver:* ${revealLink}\n`);
  ...
  const handleSend = (p: Participant, index: number) => {
    ...
    // Auto-advance after a brief delay to allow the user to see feedback
    // but only if it's not the last one
    if (index < participants.length - 1) {
      ...
      setTimeout(() => {
        setSendingIndex(index + 1); // This will push it past the end, triggering "Done" state
      }, 1000);
    }
  };
        ...
        <div className="text-center space-y-8 animate-fade-in-up py-4">
          <div className="bg-white p-8 rounded-xl shadow-lg border border-gray-100">
            ...
            <p className="text-gray-600 mb-8">
              Todos os participantes receberam seus links secretos.
            </p>
    ...
    return (
      <div className="max-w-md mx-auto space-y-6">
        <div className="bg-indigo-600 text-white p-6 rounded-xl shadow-lg text-center">
           <h2 className="text-2xl font-bold mb-2">🎁 Hora de Enviar!</h2>
           <p className="text-indigo-100 text-sm">
             Siga a sequência para enviar a mensagem secreta para cada um.
           </p>
        </div>
        ...
        <div className="bg-white p-8 rounded-xl shadow-xl border border-indigo-100 text-center relative overflow-hidden">
          ...
          <h3 className="text-xl text-gray-500 mb-1">Enviar para</h3>
          ...
          <p className="text-xs text-gray-400 mt-4">
            Abrirá o WhatsApp. Volte aqui para o próximo.
          </p>
        </div>
    ...
    <div className="space-y-6">
      <div className="bg-indigo-600 text-white p-6 rounded-xl shadow-lg text-center">
        ...
        <p className="text-indigo-100">
          Envie o <strong>Link Seguro</strong> para cada participante.
        </p>
      </div>
